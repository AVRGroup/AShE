...
def process_image(image_data, mask_data):
    ...
    marked_image = create_image_with_center_marks(combined_mask2, object_center, shadow_center)
        ...
        # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    # 1) Contar pixels do objeto real (maior objeto da cena)
    real_pixel_count = np.count_nonzero(largest_object_mask == 255)

    # 2) Contar pixels do objeto 3D (presumindo que 'mask' é binária do 3D)
    object3d_pixel_count = np.count_nonzero(mask == 255)

    # 3) Calcular fator de escala em termos de pixel count
    if object3d_pixel_count > 0:
        scaleFactorPixels =  object3d_pixel_count / real_pixel_count
    else:
        scaleFactorPixels = 1.0

    # Se quiser combinar com 'old_proportion', você pode multiplicar
    # totalScaleFactor = scaleFactorPixels * old_proportion
    # ou usar só scaleFactorPixels isolado.
    # Combinar
    totalScaleFactor = scaleFactorPixels * proportion
